...
except ImportError:  # pragma: no cover - optional dependency for ROI decoding
    cv2 = None
...
class PatientUpdate(Model):
    ...
    timestamp: str
    forehead_roi: Optional[str] = None  # base64-encoded BGR ROI focused on forehead skin
...
heart_rate_monitors: Dict[str, HeartRateMonitor] = {}
...
def _decode_forehead_roi(encoded_roi: Optional[str]) -> Optional[np.ndarray]:
    if not encoded_roi or cv2 is None:
        return None

    try:
        payload = encoded_roi.split(",")[-1]
        roi_bytes = base64.b64decode(payload)
        buffer = np.frombuffer(roi_bytes, dtype=np.uint8)
        roi = cv2.imdecode(buffer, cv2.IMREAD_COLOR)
        return roi if roi is not None and roi.size > 0 else None
    except Exception:
        return None


def _update_heart_rate_from_roi(patient_id: str, encoded_roi: Optional[str]) -> Optional[int]:
    roi = _decode_forehead_roi(encoded_roi)
    if roi is None:
        return None

    monitor = heart_rate_monitors.setdefault(patient_id, HeartRateMonitor())
    bpm = monitor.process_frame(None, roi)
    return int(bpm) if isinstance(bpm, (int, np.integer)) else None
...
def analyze_patient_metrics(patient_id: str, vitals: Dict, cv_metrics: Dict) -> Dict:
    """
    External helper so tools/scripts can reuse the same analysis pipeline.
    """
    payload = {
    ...
    return _analyze_patient_with_claude(payload)
...
async def handle_patient_update(ctx: Context, sender: str, msg: PatientUpdate):
    ...
    vitals_payload = dict(msg.vitals or {})
    cv_metrics_payload = dict(msg.cv_metrics or {})

    computed_hr = _update_heart_rate_from_roi(msg.patient_id, msg.forehead_roi)
    if computed_hr is not None:
        vitals_payload["heart_rate"] = computed_hr
    elif msg.patient_id in heart_rate_monitors:
        vitals_payload.setdefault("heart_rate", heart_rate_monitors[msg.patient_id].last_heart_rate)
    ...
    analysis = analyze_patient_metrics(msg.patient_id, vitals_payload, cv_metrics_payload)
    patients[msg.patient_id]["analysis"] = analysis
